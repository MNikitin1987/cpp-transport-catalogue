...
namespace graph {
    ...
    public:
        ...
        optional<RouteInfo> BuildRoute(VertexId from, VertexId to) const;

    private:
        struct RouteInternalData {
            Weight weight;
            optional<EdgeId> prev_edge;
        };
        using RoutesInternalData = vector<vector<optional<RouteInternalData>>>;
        ...
        void InitializeRoutesInternalData(const Graph& graph) {
            ...
            for (VertexId vertex = 0; vertex < vertex_count; ++vertex) {
                ...
                routes_internal_data_[vertex][vertex] = RouteInternalData{ ZERO_WEIGHT, nullopt };

                for (const EdgeId edge_id : graph.GetIncidentEdges(vertex)) {

                    const auto& edge = graph.GetEdge(edge_id);
                    if (edge.weight < ZERO_WEIGHT) {
                        throw domain_error("Edges' weights should be non-negative");
                    }
    ...
    Router<Weight>::Router(const Graph& graph)
        ...
        , routes_internal_data_(graph.GetVertexCount(),
            vector<optional<RouteInternalData>>(graph.GetVertexCount()))
    {
    ...
    template <typename Weight>
    optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from,
        VertexId to) const {
        ...
        vector<EdgeId> edges;
        for (optional<EdgeId> edge_id = route_internal_data->prev_edge;
            edge_id;
        ...
        return RouteInfo{ weight, move(edges) };
    }
