...
namespace tc {
	...
	pair<size_t, double> TransportCatalogue::GetRouteLength(string_view bus_name) const {
		...
		for (const auto& stop : bus.stops) {
			...
			if (bus.round_trip) {
				...
				length_road += GetDistance(name_stop1, name_stop2);
			}
			else {
				length_geo += 2.0 * geo::ComputeDistance(geo_stop1, geo_stop2);
				length_road += GetDistance(name_stop1, name_stop2);
				length_road += GetDistance(name_stop2, name_stop1);
			}
	...
	vector<string_view> TransportCatalogue::GetStopNamesSortedNoCheck() const {
		vector<string_view> res;
	...
	vector<pair<pair<string_view, string_view>, size_t>> TransportCatalogue::GetAllDistances() const {
		vector<pair<pair<string_view, string_view>, size_t>> res;

		for (const auto& [stops, distance] : stop_distances_) {
			res.push_back({{stops.first, stops.second}, distance});
		}
